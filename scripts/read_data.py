...
## transcations without fraud data
new_transaction_2 = new_transaction.filter(new_transaction.is_fraud == 'False')
...
new_transaction.write.mode('overwrite').option("header",True).csv(find_files('curated',search_path = search_path)+'/full_data_no_fraud.csv')
...
## -----------------------------------------------------------------------------------------------------------------------------------------------
## Analyze curated transction (with fraud data) file to summary merchant information
...
## -----------------------------------------------------------------------------------------------------------------------------------------------
## Analyze curated transction (without fraud data) file to summary merchant information
## Save the merchant information to file "merchant_info.csv"
## -----------------------------------------------------------------------------------------------------------------------------------------------

## format the curated dataset properly
curated_csv = new_transaction_2.withColumn('dollar_value', new_transaction_2.dollar_value.cast(FloatType())) \
    .withColumn('avg_personal_income_weekly', new_transaction_2.avg_personal_income_weekly.cast(FloatType())) \
        .withColumn('take_rate', new_transaction_2.take_rate.cast(FloatType())) \
            .withColumn('total_population', new_transaction_2.total_population.cast(IntegerType())) \
                .withColumn('is_fraud', when(new_transaction_2.is_fraud == 'False', 0).otherwise(1))
...
## Save the merchant info
merchant_info.write.mode('overwrite').option("header",True).csv(find_files('curated',search_path = search_path)+'/merchant_info_no_fraud.csv')